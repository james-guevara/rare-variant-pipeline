#!/usr/bin/env python3
"""
Filter rare missense variants using DuckDB streaming.

Usage:
    python filter_missense.py input.tsv.gz -o filtered.tsv --af 0.01 --mpc 2
    python filter_missense.py input.tsv.gz -o filtered.tsv --af 0.01 --mpc 2 --canonical
    
    # Pipeable (use - for stdin/stdout)
    zcat input.tsv.gz | python filter_missense.py - -o - --mpc 2 | gzip > out.tsv.gz
"""
...
def build_where_clause(args, gene_set: set[str] | None) -> str:
    ...
    conditions.append("Consequence LIKE '%missense%'")
    ...
    if args.mpc is not None:
        conditions.append(f"TRY_CAST(MPC_score AS DOUBLE) > {args.mpc}")
    if args.alphamissense is not None:
        conditions.append(f"TRY_CAST(AlphaMissense_score AS DOUBLE) > {args.alphamissense}")
...
def main():
    parser = argparse.ArgumentParser(description="Filter rare missense variants")
    ...
    # Pathogenicity filters
    parser.add_argument("--mpc", type=float, help="MPC threshold")
    parser.add_argument("--alphamissense", type=float, help="AlphaMissense threshold")
